/**
 * cortexOS-note.js — Note launcher (action pre-set, starts at model selection)
 * Assign to a QuickAdd macro called "cortexOS: Note"
 * Then add a Commander toolbar button → QuickAdd: cortexOS: Note
 */
"use strict";
...
module.exports = async ({ quickAddApi: { suggester, inputPrompt }, app }) => {
  ...
  const topic    = await inputPrompt("Topic?", "e.g. Newton's Laws"); if (!topic) return;
  const noteType = await suggester(d(NOTE_TYPES), v(NOTE_TYPES)); if (!noteType) return;

  const cmd = `"${scripts}/note" ${model} "${subject}" "${topic}" ${noteType}`;
  new Notice(`📝  Note · ${model} · ${subject}  —  running…`, 9000);
